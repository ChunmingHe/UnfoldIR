...
class SS2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, d: torch.Tensor):
        ...
        # ====== 关键修复：对齐 d 的 H,W 到 x 的 H,W ======
        # x: [B, inner, H, W], d: [B, inner, Hd, Wd]
...
class FeedForward(nn.Module):  ## Implicit Retinex-Aware
    ...
    def forward(self, x_in, illum):
        ...
        if illum.shape[-2:] != attn1.shape[-2:]:
            illum = F.interpolate(illum, size=attn1.shape[-2:], mode="bilinear", align_corners=False)
...
class R(nn.Module):
    ...
    def forward(self, illu, input_low_img, refl, mu1, mu2):
        H = min(illu.shape[2], input_low_img.shape[2], refl.shape[2])
        W = min(illu.shape[3], input_low_img.shape[3], refl.shape[3])
        illu = illu[:, :, :H, :W]
        input_low_img = input_low_img[:, :, :H, :W]
        refl = refl[:, :, :H, :W]
...
class L(nn.Module):
    ...
    def forward(self, illu, input_low_img, refl, lambda1, lambda2):
        ...
        illu = torch.mean(illu, dim=1, keepdim=True)  # [B,1,H,W]
        conv_illu = self.convs(illu)  # [B,3,H,W]
        if conv_illu.shape[-2:] != refl.shape[-2:]:
            HH = min(conv_illu.shape[2], refl.shape[2])
            WW = min(conv_illu.shape[3], refl.shape[3])
            conv_illu = conv_illu[:, :, :HH, :WW]
            ones_matrix = ones_matrix[:, :, :HH, :WW]
            input_low_img = input_low_img[:, :, :HH, :WW]
            refl = refl[:, :, :HH, :WW]

        A = refl ** 2 + lambda2 * ones_matrix + lambda1 * (self.w ** 2) * (conv_illu ** 2) + 1e-6
        b = refl * input_low_img + lambda2 * torch.mean(illu, dim=1, keepdim=True)
        L = b / A
        return L
...
class UnfoldIR_real(nn.Module):
    def __init__(self, nf=64,
                 ...
                 num_layer=3):
        super(UnfoldIR_real, self).__init__()

        self.nf = nf
        self.depths = depths
        self.estimator = Illumination_Estimator(2 * nf)
        self.img_size = img_size
        self.VSSB_R = nn.Sequential(*[VSSBlock1(
        ...
        self.VSSB_R2 = nn.Sequential(*[VSSBlock(
            hidden_dim=nf, norm_layer=norm_layer, d_state=d_state, expand=mlp_ratio, img_size=img_size,
            patch_size=patch_size, embed_dim=embed_dim) for i in range(self.depths[0])])
        self.conv_R = nn.Conv2d(3, nf, 3, 1, 1, bias=False)
        self.conv_R2 = nn.Conv2d(48, 3, 3, 1, 1, bias=False)
        self.L = L()
        self.R = R()
        self.lambda1 = nn.Parameter(torch.tensor([0.1]), True)
        self.lambda2 = nn.Parameter(torch.tensor([0.1]), True)
        self.mu1 = nn.Parameter(torch.tensor([0.1]), True)
        self.mu2 = nn.Parameter(torch.tensor([0.1]), True)
        self.DWT = DWT()
        self.IDWT = IDWT()
        self.backbone = Backbone(nf=48,
    ...
    def unfolding(self, input_low_img):
        for t in range(3):
            if t == 0:  # initialize
                ...
                illu_hat = self.L(illu, I, refl, self.lambda1, self.lambda2)
                refl_hat = self.R(illu, I, refl, self.mu1, self.mu2)
                illu_fea, illu = self.estimator(I, illu_hat)
                refl_hat1 = self.conv_R(refl_hat)
                ll, lh, hl, hh = self.DWT(refl_hat1)
                ll = self.VSSB_R(ll)
                lh = self.VSSB_R(lh)
                hl = self.VSSB_R(hl)
                hh = self.VSSB_R(hh)
                refl = self.IDWT(ll, lh, hl, hh)
                refl = self.VSSB_R2((refl, illu_fea))[0]
                refl = self.conv_R2(refl)
        return illu, I

    def forward(self, input_low_img):
        illu, I = self.unfolding(input_low_img)
        out = self.backbone(I, illu)
        return out
